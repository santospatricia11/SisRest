...
class TestBeneficiarioController {
	...
	@InjectMocks
	@Mock
	private static BeneficiarioResource controller;
	@Mock
	private static BeneficiarioServiceConvert beneficiarioServiceConvert;
	@InjectMocks
	@Mock
	private static BeneficiarioService service;
	@Mock
	private static BeneficiarioRepository repository;
//dto
	@Mock
	private static BeneficiarioRequest dto;
	@Mock
	private static Beneficiario beneficiario;
	@Mock
	private ResponseEntity resp;

	private Set<ConstraintViolation<BeneficiarioRequest>> violations;
	private static Validator validator;

	@BeforeAll
	public static void setup() {
		service = new BeneficiarioService();
		controller = new BeneficiarioResource();
		beneficiarioServiceConvert = new BeneficiarioServiceConvert();
		ReflectionTestUtils.setField(service, "beneficiarioRepository", repository);
		ReflectionTestUtils.setField(controller, "beneficiarioService", service);
		ReflectionTestUtils.setField(controller, "converterService", beneficiarioServiceConvert);
	...
	public void saveInvaliIdAfeterJson(long id) throws Exception {
		if (id == 0) {
			BeneficiarioRequest novo = new BeneficiarioRequest();
			resp = controller.create(novo);
		} else {
			dto.setId(id);
			resp = controller.create(dto);
		}
		assertEquals("Não foi possível usar save, o campo nome está faltando!", resp.getBody());
	}

	@Test
	 void saveInvalidNull() throws Exception { // save invalid - obj is null
	...
	@Test
	public void updateInvalidDtoNull() throws Exception { // update invalid - obj null
	...
	@Test
	public void updateInvalidIdNotFound() throws IllegalArgumentException, Exception  { // update invalid - Dto's Id not found in DB
	...
	@Test
	public void deleteInvalidIdNull() { // delete invalid - the id is 0
	...
	public void findByIDInvalidNotFound() { // findByID invalid - not found in DB
		when(repository.existsById((long) 9)).thenReturn(false);
		resp = controller.getById(1);
	...
	@Test
	public void getAllValidBeneficiarios() { // getAll valid - have no beneficiario in DB
